...
image = (
    ...
    .add_local_python_source("model", "configuration_qwen2")
)

app = modal.App()

def benchmark_my_qwen(prompt, hf_model, tokenizer, max_tokens=100):
    from model import MyQwen2ForCausalLM, generate_text
    from configuration_qwen2 import Qwen2Config

    hf_model.cpu()
    config = Qwen2Config()
    qwen = MyQwen2ForCausalLM(config).cuda()
...
def run():
    # Quick benchmark
    run_benchmark.remote("Recite the star-spangled banner", max_tokens=500, num_runs=3)
    
    # Longer benchmark
    print("\n" + "=" * 80)
    print("LONGER GENERATION BENCHMARK")
    run_benchmark.remote("Write a detailed explanation of machine learning.", max_tokens=500, num_runs=2)
